...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # Initialize variables and the variables to eliminate
        evidenceVariablesSet = set(evidenceDict.keys())
        queryVariablesSet = set(queryVariables)
        eliminationVariables = (bayesNet.variablesSet() - evidenceVariablesSet) - queryVariablesSet

        # Grab all factors where we know the evidence variables
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        ...
        for elimVariable in eliminationOrder:
            ...
            # If resulting joinedFactor has more than 1 Unconditionned Variable
            # we eleminate the variable and add the remainder to current factor  
            if len(joinedFactor.unconditionedVariables()) > 1:
                eliminatedFactor = eliminate(joinedFactor, elimVariable)
