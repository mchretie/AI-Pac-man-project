...
def constructBayesNet(gameState):
    ...
    #        (Obs1)    (Obs2)  ...   (Obs n)
    #
    # Constructing the Bayes Net
    # Creating an observation variable for each house and their walls
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVars.append(obsVar)
            edges.append((FOOD_HOUSE_VAR, obsVar)) # Linking the Obs Vars to the houses and adding to edges list
            edges.append((GHOST_HOUSE_VAR,obsVar))

    # Linking X/Y variables to houses 
    ...
    # Giving the Obs Vars their value domains
    for var in obsVars:
        variableDomainsDict[var] = OBS_VALS

    # Giving value domains to remaining variables
...
def fillYCPT(bayesNet, gameState):
    ...
    # Setting the probabilities for the different house layouts 
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    # Iterate to find best proba and save best assignement
    for assignment in assignments:
        proba = Factor.getProbability(assignment)
        if proba > bestProb:
            bestProb = proba
            bestGuess = assignment[FOOD_HOUSE_VAR] 
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        # Create factor and fetch assignements
        factor = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder) 
        assignments = factor.getAllPossibleAssignmentDicts()

        # Value collector
        values = {}
        # Vars to be used as keys
        rightVal = "rightVal"
        leftVal = "leftVal"

        # Find probability and save to corresponding key
        for assignement in assignments:
            proba = factor.getProbability(assignement)
            if proba:
                if assignement[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL or assignement[FOOD_HOUSE_VAR] == BOTTOM_RIGHT_VAL:
                    values[rightVal] = proba
                else:
                    values[leftVal] = proba

        # Determine exploration values
        rightExpectedValue = WON_GAME_REWARD * values[rightVal] + GHOST_COLLISION_REWARD *  values[leftVal]
        leftExpectedValue = GHOST_COLLISION_REWARD * values[rightVal] +  WON_GAME_REWARD *  values[leftVal]
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        exOutcomes = self.getExplorationProbsAndOutcomes(evidence)

        for elem in exOutcomes:
            actionVals = self.computeEnterValues(elem[1],  enterEliminationOrder)
            expectedValue += elem[0] * max(actionVals)
