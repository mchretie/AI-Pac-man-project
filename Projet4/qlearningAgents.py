...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state) -> Optional[str]:
        ...
        for action in self.getLegalActions(state):
          tmpScore = self.getQValue(state, action)
          if maxValue < tmpScore:
            maxValue = tmpScore
            max_action = action
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.Qvalues[(state, action)] = (1-self.alpha) * Qvalue + self.alpha * newValue
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for key in self.weights:
          res += self.weights[key] * features[key]
