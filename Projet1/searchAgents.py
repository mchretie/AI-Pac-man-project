...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        # Flags to record visited corners
        # string of four "0" or "1" (start = "0000", goal = "1111")
        # Index correspondong to problem.corners
        self.cornersFlag ="0000"
        # Combining state position with flags give us a dict. key structure allowing 
        # us to find a path with multiple stages/objectives
        self.state = [self.startingPosition, self.cornersFlag]
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Verifies if all corner flags set to visited
        return state[1] == '1111'
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            tmpChild = self.getNextState(state, action)
            cost = self.getActionCost(state, action, tmpChild)
            # Bundle all together save 
            children.append([tmpChild, action, cost])
    ...
    def getNextState(self, state, action):
        ...
        # Preparing nextState and flags
        nextState = (nextx, nexty)
        tmpFlags = state[1]

        # Verifie if nextState not a corner
        if nextState in self.corners:
            index = self.corners.index(nextState)                   # get index for flag
            tmpFlags = tmpFlags[0:index]+ "1" +tmpFlags[index+1:5]  # update flag
        return [nextState, tmpFlags]
...
def cornersHeuristic(state, problem):
    ...
    cornersFlag = state[1]
    toVisit = []
    # Selecting corners yet to be visited
    for i in range(4):
        if cornersFlag[i] == "0":  
            toVisit.append(i)

    # Calculating their heuristic values
    heurVals =[]
    for corner in toVisit:
        problem.goal = corners[corner]
        heurVals.append(manhattanHeuristic(state[0], problem))
    # Returning max heuristic value
    hn=0
    if len(heurVals)>0:
        hn = max(heurVals)
    return hn 
...
def foodHeuristic(state, problem):
    ...
    # Calculating Heuristic Values for every food with mazeDistance 
    for food in foodList:        
        heurVals.append(mazeDistance(position, food, problem.startingGameState))

        # First try with manhattan yielded 3/4
        #problem.goal = food
        #heurVals.append(manhattanHeuristic(position, problem))
